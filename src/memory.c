...
static void error_handler(MEM_Controller controller, char* filename, 
	int line, char* msg) {
	...
	if (controller->fail_mode == MEM_FAIL_AND_RETURN) {
		exit(1);
	}
}
...
#ifdef DEBUG
static void chain_block(MEM_Controller controller, 
	Header* new_header) {
...
void* MEM_malloc_func(MEM_Controller controller, char* filename, \
...
void* MEM_realloc_func(MEM_Controller controller, char* filename,
